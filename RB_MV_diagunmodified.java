...
public class RB_MV_diagunmodified{
...
public static double AxinDiag(double[] val, int[] colindex, int[] rowindex, int [] pointer,  int[] diag, double [] x, int n){
  double [] y5= new double[n];
 
  double startTime5 = System.currentTimeMillis();
   for (int i=0; i<diag.length; i++){
    int start=pointer[i];
    int end=pointer[i+1]-1;
        for (; start<=end; start++ ){
           double scalar= val[start];
           int jcol=colindex[start];
           int irow=rowindex[start];
           y5[irow] += scalar * x[jcol];
        }
      }
...
public static double ATxinDiag(double[] val, int[] colindex, int[] rowindex, int [] pointer,  int[] diag, double [] x, int n){
...
// public static void main(String[] args){
   ...
   public static void main(String[] args) throws Throwable {
     PrintStream out = new PrintStream(new FileOutputStream("BBMV_test10k-100k.xlsx"));
     System.setOut(out);

    System.out.println("n" + "," + "#diags"+ "," + "%of nnz" + "," + "Ax_CRS" + ","+ "Ax_JSA" +","+ "Ax_Diag"+ "," +"ATx_CRS" +","+ "ATx_JSA"+ ","+"ATx_Diag"); 
  ...
  double[][] val_JSA=(double[][]) CRSObjects[3];
   ...
   //Time of Ax in CRS, JSA, Diag
      ...
      double t1_Diag= AxinDiag( val_Diag, colind_Diag, rowind_Diag, pointer_Diag, diag, x, n);
   ...
   //time of A'x in CRS, JSA, Diag
     double t2_CRS=ATxinCRS(val_CRS, colind_CRS, rowptr_CRS, x, n );
      ...
      double t2_Diag= ATxinDiag( val_Diag, colind_Diag, rowind_Diag, pointer_Diag, diag, x, n);
